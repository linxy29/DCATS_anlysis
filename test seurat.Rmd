...
names(listSamples) = c("batch1", "batch5", "batch4", "batch3", "batch2", "batch6")


# log-normalization and identify variable features
for (i in 1:length(listSamples)) {
  listSamples[[i]]@meta.data$batch = NULL
...
Problem not solved. Try to change batch at the begining
...
simMut_mat = sim_list$simMut_mat
batchNor = sim_list$batchNor %>% 
  plyr::mapvalues(from = c("Nor1", "Nor2", "Nor3"), to = c("Sa", "Sc", "Sb"))
batchMut = sim_list$batchMut%>% 
