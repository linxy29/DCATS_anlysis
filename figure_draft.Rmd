...
filenames = filenames[grep("K8_con100_splatter1500&2500para.RData", filenames)]
filenames
```
...
for (file in filenames) {
  ...
  for (i in 1:length(select_col)){
    ...
    if (i == 1) {pred_res = ifelse(pred < 0.05, "P", "N")}
    else {pred_res = ifelse(pred < 0.05, "P", "N")}
...
nhoodsH1_specificity = nhoodsRes_all %>% 
  ...
  ggplot(aes(replicates, method)) +
    ...
    scale_fill_gradient(low = "white", high = "red", limits = c(0.9,1)) +
  xlab("# of Replicates") +
  theme(axis.title.y = element_blank(),
        axis.text.y = element_blank())

nhoodsH1_auc + nhoodsH1_mcc + nhoodsH1_sensitivity + nhoodsH1_specificity
ggsave("./plot/supFig1B4.png", bg = "transparent", width = 10, height = 6)
```
...
filenames = filenames[grep("para", filenames)]
filenames
```
...
save(nhoodsRes_all, file = "D:/Data/DCATS/simulation/nhoodsRes_all_clusterN.RData")
```

```{r}
load("D:/Data/DCATS/simulation/nhoodsRes_all_clusterN.RData")
nhoodsRes_all %>% 
  mutate(clustersN = ifelse(is.na(clustersN), "K8", clustersN),
         clustersN = str_remove(clustersN, "K"),
         clustersN = factor(clustersN, levels = c("8", "10", "12"))) %>% 
...
nhoodsH1_auc + nhoodsH1_mcc + nhoodsH1_sensitivity + nhoodsH1_specificity
ggsave("./plot/supFig1B5.png", bg = "transparent", width = 10, height = 6)
```
...
for (file in fileL) {
  ...
  for (i in 4:(3+numb_mthd)) {
    ...
    if (colnames(simulationDF)[i] == "milo_pct") {
      pred_res = ifelse(simulationDF$milo_diff > 2, "P", "N")
...
```{r}
p1B2 = res_all %>% 
  dplyr::select(method, prauc, factor) %>% 
  pivot_wider(names_from = factor, values_from = prauc) %>% 
  pivot_longer(condition:gender, names_to = "factor", values_to = "prauc") %>% 
  filter(method %in% c("speckle", "diffcyt", "fisher", "scDC", "estPhi_emK", "milo")) %>%
  mutate(method = ifelse(method == "fisher", "Fisher", method),
         method = ifelse(method == "estPhi_emK", "DCATS", method)) %>% 
  mutate(method = factor(method, level = c("scDC", "milo", "speckle", "diffcyt", "Fisher", "DCATS")),
         factor = factor(factor, level = c("condition", "age", "gender"))) %>% 
  ggplot(aes(factor, method)) +
    geom_tile(aes(fill = prauc)) + 
    geom_text(aes(label = ifelse(is.na(prauc), "N.A.", as.character(round(prauc, 3))))) +
...
p1B2_sup = res_all %>% 
  ...
  filter(method %in% c("speckle", "diffcyt", "fisher", "scDC", "betabin_null", "estPhi_null", "wtoPhi_emK", "estPhi_emK", "milo")) %>%
  mutate(method = ifelse(method == "fisher", "Fisher", method)) %>% 
  mutate(method = factor(method, level = c("scDC", "milo", "speckle", "diffcyt", "Fisher", "estPhi_null", "estPhi_emK", "betabin_null", "wtoPhi_emK")),
         factor = factor(factor, level = c("condition", "age", "gender"))) %>% 
  ggplot(aes(factor, method)) +
    geom_tile(aes(fill = prauc)) + 
    geom_text(aes(label = ifelse(is.na(prauc), "N.A.", as.character(round(prauc, 3))))) +
