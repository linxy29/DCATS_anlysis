...
ggsave("./plot/supFig1A2.png", bg = "transparent", width = 8, height = 5)
```
...
filenames = filenames[grep("K8_con100_splatter1500&2500.RData", filenames)]
filenames
```
...
for (file in filenames) {
  ...
  method = colnames(simulationDF)[3:dim(simulationDF)[2]] %>% str_replace("_pavls", "_pvals") %>% str_remove("_pvals")
  numb_mthd = length(method)
  mcc = rep(NA, numb_mthd)
  auc = rep(NA, numb_mthd)
  prauc = rep(NA, numb_mthd)
...
res_all %>% 
  filter(method %in% c("estPhi_null", "estPhi_emK", "fisher", "speckle", "diffcyt", "betabin_null", "wtoPhi_emK", "scDC")) %>% 
...
main_p1 = res_all %>% 
  ...
  mutate(method = ifelse(method == "fisher", "Fisher", method)) %>% 
  mutate(method = factor(method, level = c("scDC", "speckle", "diffcyt", "Fisher", "estPhi_emK")),
...
main_p3 = res_all %>% 
  ...
  mutate(method = factor(method, level = c("scDC", "speckle", "diffcyt", "Fisher", "DCTAS"))) %>% 
...
p1B2_sup = res_all %>% 
  ...
  ggplot(aes(factor, method)) +
    ...
    scale_fill_gradient(low = "white", high = "red", limits = c(0.5,0.9))+
  xlab("Covariates")
ggsave("./plot/supFig1C1.png", bg = "transparent")
```
...
all_resC3 %>% 
  ...
  pivot_longer(truth:speckle_Res, names_to = "method", values_to = "res") %>% 
  pivot_wider(names_from = cluster, values_from = res) %>% 
  mutate(method = str_remove(method, "_Res")) %>% 
  filter(method %in% c("truth", "estPhi_emSVM", "fisher", "scDC", "speckle")) %>% 
...
```{r}
p1 = ggplot(data = Ren2021_plotDF[Ren2021_plotDF$clusterRes %in% c("CD8+ T cells", "Monocytes", "CD4+T cells", "B cells", "NK cells"),], aes(x = state, y = proportion)) +
    geom_boxplot(aes(color = state)) +
    theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) + 
    facet_grid(.~factor(clusterRes, levels=c("CD8+ T cells", "Monocytes", "CD4+T cells", "B cells", "NK cells"))) +
    theme(legend.position="none")


p2 = ggplot(data = Ren2021_plotDF[Ren2021_plotDF$clusterRes %!in% c("CD8+ T cells", "Monocytes", "CD4+T cells", "B cells", "NK cells"),], aes(x = state, y = proportion)) +
    geom_boxplot(aes(color = state)) +
    theme(axis.text.x=element_blank(),
          axis.ticks.x=element_blank()) +
    facet_grid(.~factor(clusterRes, levels=c("Megakaryocytes", "Plasma", "Macrophages", "Dendritic cells", "Neutrophils"))) + 
    theme(legend.position="bottom") +
...
stat.test = stat.test1 %>% rbind(stat.test2) %>% rbind(stat.test3) %>% rbind(stat.test4) %>% 
  ...
  mutate(clusterRes = ifelse(clusterRes=="B", "B cells", clusterRes),
         ...
         clusterRes = ifelse(clusterRes=="Macro", "Macrophages", clusterRes),
         clusterRes = ifelse(clusterRes=="Neu", "Neutrophils", clusterRes)) %>% 
  mutate(color = ifelse(!((clusterRes == "CD8+ T cells"&group1 == "control"&group2 == "mild/moderate (progression)") | (clusterRes == "Monocytes"&group1 == "mild/moderate (convalescence)"&group2 == "mild/moderate (progression)")), "black", "red2"))

```

```{r}
pB1 = ggplot(data = Ren2021_plotDF[Ren2021_plotDF$clusterRes %in% c("CD8+ T cells", "Monocytes", "CD4+T cells", "B cells", "NK cells"),], aes(x = state, y = proportion)) +
    geom_boxplot(aes(color = state)) +
    theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) + 
...
pB2 = ggplot(data = Ren2021_plotDF[Ren2021_plotDF$clusterRes %!in% c("CD8+ T cells", "Monocytes", "CD4+T cells", "B cells", "NK cells"),], aes(x = state, y = proportion)) +
    geom_boxplot(aes(color = state)) +
    theme(axis.text.x=element_blank(),
          axis.ticks.x=element_blank()) +
    facet_grid(.~factor(clusterRes, levels=c("Megakaryocytes", "Plasma", "Macrophages", "Dendritic cells", "Neutrophils"))) + 
...
stat.testp1 = stat.test %>% 
  ...
  filter(clusterRes %in% c("CD8+ T cells", "Monocytes", "CD4+T cells", "B cells", "NK cells"))
...
(pA1+pA2+pA3+pA4)/(pB1 + add_pvalue(stat.testp1, tip.length = 0.015, step.increase = 0.03, colour = "color"))/(pB2  + add_pvalue(stat.testp2, tip.length = 0.015, step.increase = 0.03, colour = "color"))
ggsave("./plot/Fig2.png", width = 10, height = 15)
(pB1 + add_pvalue(stat.testp1, tip.length = 0.015, step.increase = 0.03, colour = "color"))/(pB2  + add_pvalue(stat.testp2, tip.length = 0.015, step.increase = 0.03, colour = "color"))
ggsave("./plot/Fig2C.png", width = 10, height = 10)
```
