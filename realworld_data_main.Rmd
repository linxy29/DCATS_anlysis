...
```{r}
dcats_GLM <- function(count_mat, design_mat, similarity_mat=NULL, n_samples=50, pseudo_count=NULL,  base_model='NULL', phi=NULL) {
...
avrgPhi = getPhi2(countM, designM)$avrgPhi
adjPhi = getPhi2(countM, designM)$adjPhiV
estPhi_null = dcats_GLM(countM, designM, phi = rep(estPhi, ncol(countM)))
estPhi_emSVM = dcats_GLM(countM, designM, similarity_mat = Haber2017$svm_mat, n_samples = NULL, phi = rep(estPhi, ncol(countM)))
avrgPhi_null = dcats_GLM(countM, designM, phi = rep(avrgPhi, ncol(countM)))
avrgPhi_emSVM = dcats_GLM(countM, designM, similarity_mat = Haber2017$svm_mat, n_samples = NULL, phi = rep(avrgPhi, ncol(countM)))
adjPhi_null = dcats_GLM(countM, designM, phi = adjPhi)
adjPhi_emSVM = dcats_GLM(countM, designM, similarity_mat = Haber2017$svm_mat, n_samples = NULL, phi = adjPhi)
## Fisher's exact test
...
all_resC3 = data.frame(cluster = rownames(betabin_null$LRT_pvals), truth = c(rep("N", 7), "P")) %>% 
  mutate(betabin_null_pvals = as.vector(betabin_null$LRT_pvals),
         ...
         avrgPhi_null_pvals = as.vector(avrgPhi_null$LRT_pvals),
         avrgPhi_emSVM_pvals = as.vector(avrgPhi_emSVM$LRT_pvals),
         adjPhi_null_pvals = as.vector(adjPhi_null$LRT_pvals),
         adjPhi_emSVM_pvals = as.vector(adjPhi_emSVM$LRT_pvals),
  ...
  #dplyr::select(truth, estPhi_emSVM_Res, estPhi_emSVM_pvals)
  mutate(avrgPhi_null_Res = ifelse(avrgPhi_null_pvals < 0.01, "***", "n.s."),
         avrgPhi_null_Res = ifelse(0.01<avrgPhi_null_pvals&avrgPhi_null_pvals<0.05, "**", avrgPhi_null_Res),
         avrgPhi_null_Res = ifelse(0.05<avrgPhi_null_pvals&avrgPhi_null_pvals<0.1, "*", avrgPhi_null_Res)) %>%
  mutate(avrgPhi_emSVM_Res = ifelse(avrgPhi_emSVM_pvals < 0.01, "***", "n.s."),
         avrgPhi_emSVM_Res = ifelse(0.01<avrgPhi_emSVM_pvals&avrgPhi_emSVM_pvals<0.05, "**", avrgPhi_emSVM_Res),
         avrgPhi_emSVM_Res = ifelse(0.05<avrgPhi_emSVM_pvals&avrgPhi_emSVM_pvals<0.1, "*", avrgPhi_emSVM_Res)) %>%
  mutate(adjPhi_null_Res = ifelse(adjPhi_null_pvals < 0.01, "***", "n.s."),
         adjPhi_null_Res = ifelse(0.01<adjPhi_null_pvals&adjPhi_null_pvals<0.05, "**", adjPhi_null_Res),
         adjPhi_null_Res = ifelse(0.05<adjPhi_null_pvals&adjPhi_null_pvals<0.1, "*", adjPhi_null_Res)) %>%
  mutate(adjPhi_emSVM_Res = ifelse(adjPhi_emSVM_pvals < 0.01, "***", "n.s."),
         adjPhi_emSVM_Res = ifelse(0.01<adjPhi_emSVM_pvals&adjPhi_emSVM_pvals<0.05, "**", adjPhi_emSVM_Res),
         adjPhi_emSVM_Res = ifelse(0.05<adjPhi_emSVM_pvals&adjPhi_emSVM_pvals<0.1, "*", adjPhi_emSVM_Res)) %>%
...
res_GLM <- scdney::fitGLM(res_scDC, c(rep("cond1",dim(Kang2017$count_ctrl)[1]*cluster_num),rep("cond2",dim(Kang2017$count_stim)[1]*cluster_num)), pairwise = FALSE, fixed_only = TRUE, verbose = FALSE)
scDCRes_temp = summary(res_GLM$pool_res_fixed)
scDCRes = scDCRes_temp[c(cluster_num+1,(dim(scDCRes_temp)[1]-cluster_num+2):dim(scDCRes_temp)[1]),]
```
...
countM = Ren2021_cell %>% 
  filter(sample_id %in% pbmc_sampleL) %>% 
  mutate(major_type == ifelse(major_type == "Mono"&contains(celltype, "CD14"), "CD14Mono", major_type))
  group_by(sample_id, major_type) %>% 
  summarise(n = n()) %>% 
  pivot_wider(names_from = major_type, values_from = n) %>% 
  arrange(sample_id) %>% 
