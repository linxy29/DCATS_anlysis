...
ref_group = detect_reference(countM, designM, similarity_mat = Haber2017$svm_mat, fix_phi = estPhi)
estPhi_ref = dcats_GLM(countM, designM, similarity_mat = Haber2017$svm_mat, fix_phi = estPhi, reference = ref_group$celltype[1:3])
## ANCOMBC
assay_data = countM %>% t()
ancombcRes = getANCOMBC(assay_data, designM)
## Fisher's exact test
...
```{r}
#load("/storage/holab/linxy/DCATS/real_world/Exp7_Adam_2017/Haber2017_res.RData")      # sbms server
load("/Users/linxy29/Documents/Data/DCATS/real_world/Exp7_Adam_2017/Haber2017_res_v2.RData")    # my labtop
```
...
Add ANCOMBC and DCATS_ref

```{r}
all_resC3 = all_resC3 %>% 
  mutate(estPhi_ref_pvals = as.vector(estPhi_ref$LRT_pvals),
         ancombc_pvals = as.vector(ancombcRes$q_conditiong2)) %>% 
  mutate(estPhi_ref_Res = ifelse(estPhi_ref_pvals < 0.01, "***", "n.s."),
         estPhi_ref_Res = ifelse(0.01<estPhi_ref_pvals&estPhi_ref_pvals<0.05, "**", estPhi_ref_Res),
         estPhi_ref_Res = ifelse(0.05<estPhi_ref_pvals&estPhi_ref_pvals<0.1, "*", estPhi_ref_Res)) %>%
  mutate(ancombc_Res = ifelse(ancombc_pvals < 0.01, "***", "n.s."),
         ancombc_Res = ifelse(0.01<ancombc_pvals&ancombc_pvals<0.05, "**", ancombc_Res),
         ancombc_Res = ifelse(0.05<ancombc_pvals&ancombc_pvals<0.1, "*", ancombc_Res))
all_resC3
```
...
estPhi_emSVM = dcats_GLM(countM, designM, similarity_mat = Haber2017$svm_mat, fix_phi = estPhi)
ref_group = detect_reference(countM, designM, similarity_mat = Haber2017$svm_mat, fix_phi = estPhi)
estPhi_ref = dcats_GLM(countM, designM, similarity_mat = Haber2017$svm_mat, fix_phi = estPhi, reference = ref_group$celltype[1:4])
## ANCOMBC
assay_data = countM %>% t()
ancombcRes = getANCOMBC(assay_data, designM)
## Fisher's exact test
...
```{r}
save(all_resC3, all_resC10, all_resCS, file = "/Users/linxy29/Documents/Data/DCATS/real_world/Exp7_Adam_2017/Haber2017_res.RData")
```
...
estPhi_emSVM = dcats_GLM(countM, designM, similarity_mat = Haber2017$svm_mat, fix_phi = estPhi)
ref_group = detect_reference(countM, designM, similarity_mat = Haber2017$svm_mat, fix_phi = estPhi)
estPhi_ref = dcats_GLM(countM, designM, similarity_mat = Haber2017$svm_mat, fix_phi = estPhi, reference = ref_group$celltype[1:2])
## ANCOMBC
assay_data = countM %>% t()
ancombcRes = getANCOMBC(assay_data, designM)
## Fisher's exact test
...
```{r}
load("/Users/linxy29/Documents/Data/DCATS/real_world/Exp6_Kang/Kang2017_res.RData")    # my labtop
all_res = all_res %>% 
  mutate(estPhi_ref_pvals = as.vector(estPhi_ref$LRT_pvals),
         ancombc_pvals = as.vector(ancombcRes$q_conditiong2)) %>% 
  mutate(estPhi_ref_Res = ifelse(estPhi_ref_pvals < 0.01, "***", "n.s."),
  ...
  mutate(ancombc_Res = ifelse(ancombc_pvals < 0.01, "***", "n.s."),
         ...
         ancombc_Res = ifelse(0.05<ancombc_pvals&ancombc_pvals<0.1, "*", ancombc_Res))
all_res
save(all_res, file = "/Users/linxy29/Documents/Data/DCATS/real_world/Exp6_Kang/Kang2017_res.RData")
```
...
dim(svm_mat)
#svm_mat = cbind(svm_mat, rep(0,33), rep(0,33))
#colnames(svm_mat) = c(colnames(svm_mat)[1:31], "Gamma-Delta_T_cells", "low_quality_cells")
```
...
countM = meta_Schiller2019 %>% 
  ...
  dplyr::select(-`NA`) %>% 
  #dplyr::select(-`NA`,- `Gamma-Delta_T_cells`, -low_quality_cells) %>% 
...
```{r}
svm_mat = svm_mat[colnames(countM), colnames(countM)]
betabin_null = dcats_GLM(countM, designM)
...
```{r}
cbind(betabin_null$LRT_pvals , betabin_null$fdr , betabin_null_ref$LRT_pvals , betabin_null_ref$fdr)
