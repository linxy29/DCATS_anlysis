...
nhood_countL = vector(mode = "list", length = simulation_times) # list contains the count table of each true cluster
...
for (sim in 1:simulation_times){
  ...
  ## True
  wtoPhi_emT = dcats_GLM(sim_count, sim_design, simil_matU)
  estPhi_emT = dcats_GLM(sim_count, sim_design, simil_matU, fix_phi = phi)
  ## svm
  ...
  ## subset data
  posCluster = all_res %>% filter(truth == "P") %>% .$cluster
...
startTime = Sys.time()
set.seed(1)
oper <- foreach(sim = 1:simulation_times, .packages=c("tidyverse", "Seurat", "DCATS", "speckle", "diffcyt", "scdney", "tidymodels", "miloR", "SingleCellExperiment"), .verbose = TRUE) %dopar% {
  print(sim)
