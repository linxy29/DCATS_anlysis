...
```{r}
cell_pool = "discrete"  # cell pool used, can be selected from ("mis1", "discrete", "realWorld")
...
ResDF = simulationDF %>% 
  mutate(betabin_noBC_Res = ifelse(betabin_noBC_pvals < 0.05, "P","N"),
         ...
         betabin_wBCT_Res = ifelse(betabin_wBCT_pvals < 0.05, "P","N"),
         betabin_wBCK_Res = ifelse(betabin_wBCK_pvals < 0.05, "P","N"),
         ...
         betabin_wMT_Res = ifelse(betabin_wMT_pvals < 0.05, "P","N"),
         betabin_wMK_Res = ifelse(betabin_wMK_pvals < 0.05, "P","N"),
         ...
         bin_wMT_Res = ifelse(bin_wMT_pvals < 0.05, "P","N"),
         bin_wMK_Res = ifelse(bin_wMK_pvals < 0.05, "P","N"),
         ...
         diffcyt_Res = ifelse(diffcyt_pvals < 0.05, "P","N"),) %>% 
...
for (i in 1:length(methodL)) {
  ...
  pred = ResDF[, i]
  TP <- which(truth==pred&pred=="P") %>% length()
  TN <- which(truth==pred&pred=="N") %>% length()
  FP <- which(truth!=pred&pred=="P") %>% length()
  FN <- which(truth!=pred&pred=="N") %>% length()
...
## Use SPARSim to do the simulation
...
```{r}
cell_pool = "SPARSim"  # cell pool used, can be selected from ("mis1", "discrete", "realWorld")
cluster_num = 3  # numbers of clusters
concentration = 90 # indicate how simulated proportion far away from true, the larger the closer
setresolu = 0.4
rep1 = 2 
