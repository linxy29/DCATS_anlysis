...
```{r}
dcats_GLM <- function(count_mat, design_mat, similarity_mat=NULL, n_samples=50, pseudo_count=NULL,  base_model='NULL', fix_phi=NULL) {
  # Output matrices
  ...
  for (k in seq_len(ncol(design_mat))) {    ## for each factor
    ...
    for (ir in seq_len(n_samples)) {          ## for each sampling
      for (m in seq_len(ncol(count_use))) {       ## for each cluster
        ...
        if (length(fm1@varparam) < 4 || is.na(fm1@varparam[2, 2])) {next}  ## need to test here.
...
set.seed(123)
DCATS::dcats_GLM(count_mat = sim_count, design_mat = sim_design, similarity_mat = simil_mat, base_model='FULL')
## Using 1 factor vs NULL factor testing
