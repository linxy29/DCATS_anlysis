...
dcats_GLM <- function (count_mat, design_mat, similarity_mat = NULL, n_samples = 50, 
    pseudo_count = NULL, base_model = "NULL", fix_phi = NULL) 
{
    ...
    if (!is.null(similarity_mat)) {
        for (i in seq_len(nrow(count_mat))) {
            count_latent[i, ] <- sum(count_mat[i, ]) * multinom_EM(count_mat[i, 
    ...
    if (!is.null(n_samples) && !is.null(similarity_mat)) {
        ...
        for (i in seq_len(nrow(count_mat))) {
            ...
            for (j in seq_len(K)) {
                count_use[idx, ] <- (count_use[idx, ] + t(rmultinom(n_samples, 
                  count_latent[i, j], similarity_mat[j, ])))
            }
        }
    } else {count_use <- count_mat}
    ...
    for (k in seq_len(ncol(design_mat))) {
        ...
        for (ir in seq_len(n_samples)) {
            for (m in seq_len(ncol(count_use))) {
                idx <- seq(1, nrow(count_use), n_samples) + ir - 
                  1
                ...
                if (base_model == "NULL" | ncol(design_mat) == 
                  ...
                  formula_fm1 <- as.formula(paste0("cbind(n1, total-n1)", 
                  ...
                  formula_fm0 <- as.formula(paste0("cbind(n1, total-n1)", 
                    " ~ 1 + ", fm0_right, sep = ""))
                  formula_fm1 <- as.formula(paste0("cbind(n1, total-n1)", 
                    " ~ 1 + ", fm1_right, sep = ""))
                }
    ...
    res <- list(ceoffs = coeffs, coeffs_err = coeffs_err, pvals = pvals, 
        LR_vals = LR_vals, LRT_pvals = LRT_pvals, fdr = LRT_fdr)
    res
}
```

```{r}
eachPhi <- function(count_mat, design_mat, similarity_mat=NULL, n_samples=50, pseudo_count=NULL,  base_model='NULL', fix_phi=NULL) {
  # Output matrices
  coeffs     <- matrix(NA, ncol(count_mat), ncol(design_mat))
  coeffs_err <- matrix(NA, ncol(count_mat), ncol(design_mat))
  LR_vals    <- matrix(NA, ncol(count_mat), ncol(design_mat))
...
dcats_GLM(count_mat = sim_count, design_mat = sim_design, similarity_mat = simil_mat, n_samples = NULL)$LRT_pvals
eachPhi(count_mat = sim_count, design_mat = sim_design, similarity_mat = simil_mat, n_samples = NULL)$LRT_pvals
...
## Using 1 factor vs NULL factor testing
dcats_GLM(count_mat = sim_count, design_mat = sim_design, similarity_mat = simil_mat, fix_phi = 1/sum(70+1))
## No resampling and fixphi
dcats_GLM(count_mat = sim_count, design_mat = sim_design, fix_phi = 1/sum(70+1))
```
