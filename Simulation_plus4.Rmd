...
1. Different number of cells(Don't work)

2. Amplify(uniform distribution)(Don't work)

3. Different drop-out rate(have no influence on results)

Since fisher and DCATS don't consider batch effec. The changes on batch effect won't influence the results.
...
# normal sample
set.seed(234)
sample_sizeNor = runif(3, 0.7, 0.9) %>% round(3)
...
origLabels_Nor = sim_list$origLabels[1:(batch_size*3)][Nor_idx]

# mutate sample
sample_sizeMut = runif(3, 0.7, 0.9) %>% round(3)
...
origLabels_Mut = sim_list$origLabels[1:(batch_size*3)][Mut_idx]
...
```{r}
#integratedSamples = runSeurat(sim_list, batch_size, setresolu)
#saveRDS(integratedSamples, "./data/integratedSamples_addnoise5.rds")
integratedSamples = readRDS("./data/integratedSamples_addnoise5.rds")
#sendEmail("Simulation is done!")
```
