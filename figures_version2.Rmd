...
```{r}
roc_dcats = getROC(clusterRes$truth, clusterRes$estPhi_emK_pvals)$df %>% 
  mutate(fdr = 1-precision) %>% 
  filter(fdr < 1) %>% 
  dplyr::select(sensitivity, fdr) %>% 
  mutate(method = "DCATS")
roc_fisher = getROC(clusterRes$truth, clusterRes$fisher_pvals)$df %>% 
  mutate(fdr = 1-precision) %>% 
  filter(fdr < 1) %>% 
  dplyr::select(sensitivity, fdr) %>% 
  mutate(method = "fisher")
roc_scDC = getROC(clusterRes$truth, clusterRes$scDC_pvals)$df %>% 
  mutate(fdr = 1-precision) %>% 
  filter(fdr < 1) %>% 
  dplyr::select(sensitivity, fdr) %>% 
  mutate(method = "scDC")
roc_speckle = getROC(clusterRes$truth, clusterRes$speckle_pvals)$df %>% 
  mutate(fdr = 1-precision) %>% 
  filter(fdr < 1) %>% 
  dplyr::select(sensitivity, fdr) %>% 
  mutate(method = "speckle")
roc_diffcyt = getROC(clusterRes$truth, clusterRes$diffcyt_pvals)$df %>% 
  mutate(fdr = 1-precision) %>% 
  filter(fdr < 1) %>% 
  dplyr::select(sensitivity, fdr) %>% 
  mutate(method = "diffcyt")
roc_milo = getROC(clusterRes$truth, clusterRes$milo_pct)$df %>% 
  mutate(fdr = 1-precision) %>% 
  filter(fdr < 1) %>% 
  dplyr::select(sensitivity, fdr) %>% 
  mutate(method = "milo")
roc_ancombc = getROC(clusterRes$truth, clusterRes$ancombc_pvals)$df %>% 
  mutate(fdr = 1-precision) %>% 
  filter(fdr < 1) %>% 
  dplyr::select(sensitivity, fdr) %>% 
  mutate(method = "ancombc")
roc_ancombc_bs = getROC(clusterRes$truth, clusterRes$ancombc_knn_pvals)$df %>% 
  mutate(fdr = 1-precision) %>% 
  filter(fdr < 1) %>% 
  dplyr::select(sensitivity, fdr) %>% 
  mutate(method = "bcancombc")

rocDF = rbind(roc_dcats, roc_fisher, roc_scDC, roc_speckle, roc_diffcyt, roc_milo, roc_ancombc, roc_ancombc_bs)
```

```{r}
getFDR = function(sens, rocDF){
  temp = rocDF %>% arrange(desc(sensitivity)) %>% filter(sensitivity <= sens)
  return(temp$fdr[1])
}
sensitivity = seq(0.02, 1, 0.02)
dcats_fdr = rep(NA, length(sensitivity))
fisher_fdr = rep(NA, length(sensitivity))
scDC_fdr = rep(NA, length(sensitivity))
speckle_fdr = rep(NA, length(sensitivity))
diffcyt_fdr = rep(NA, length(sensitivity))
milo_fdr = rep(NA, length(sensitivity))
ancombc_fdr = rep(NA, length(sensitivity))
bcancombc_fdr = rep(NA, length(sensitivity))
for (i in 1:length(sensitivity)){
  dcats_fdr[i] = getFDR(sensitivity[i], roc_dcats)
  fisher_fdr[i] = getFDR(sensitivity[i], roc_fisher)
  scDC_fdr[i] = getFDR(sensitivity[i], roc_scDC)
  speckle_fdr[i] = getFDR(sensitivity[i], roc_speckle)
  diffcyt_fdr[i] = getFDR(sensitivity[i], roc_diffcyt)
  #milo_fdr[i] = getFDR(sensitivity[i], roc_milo)
  ancombc_fdr[i] = getFDR(sensitivity[i], roc_ancombc)
  bcancombc_fdr[i] = getFDR(sensitivity[i], roc_ancombc_bs)
}
rocDF = data.frame(sensitivity, dcats_fdr, fisher_fdr, scDC_fdr, speckle_fdr, diffcyt_fdr, ancombc_fdr, bcancombc_fdr)
rocDF[is.na(rocDF)] <- 0
rocDF = rocDF %>% 
  pivot_longer(dcats_fdr:bcancombc_fdr, names_to = "method", values_to = "fdr") %>% 
  mutate(method = str_remove(method, "_fdr"))
```

```{r}
curve = rocDF %>% 
  arrange(desc(sensitivity)) %>% 
  ggplot(aes(x = fdr, y = sensitivity, color = method)) +
...
sim2_statistics_replicates = res_all %>% 
  ...
  mutate(method = factor(method, level = c("scDC", "ancombc", "bcancombc", "milo", "wtoPhi_emK", "wtoPhi_wtoEM", "speckle", "diffcyt", "fisher", "estPhi_emK", "estPhi_wtoEM"))) %>% 
...
sim2_statistics_clustersN = res_all %>% 
  ...
  mutate(method = ifelse(method == "estPhi_null", "estPhi_wtoEM", method),
         method = ifelse(method == "betabin_null", "wtoPhi_wtoEM", method)) %>% 
  mutate(method = factor(method, level = c("scDC", "milo", "wtoPhi_emK", "wtoPhi_wtoEM", "speckle", "diffcyt", "fisher", "estPhi_emK", "estPhi_wtoEM"))) %>% 
...
ggsave("./plot/sim2_statistics.png", bg = "transparent", height = 8, width = 14)
```
...
res_all %>% 
  ...
  mutate(method = ifelse(method == "estPhi_null", "estPhi_wtoEM", method),
         method = ifelse(method == "betabin_null", "wtoPhi_wtoEM", method)) %>% 
  mutate(method = factor(method, level = c("milo", "scDC", "speckle", "diffcyt", "Fisher", "estPhi_wtoEM", "estPhi_emK", "wtoPhi_wtoEM", "wtoPhi_emK")),
         factor = factor(factor, level = c("condition", "age", "gender"))) %>% 
  ggplot(aes(factor, method)) +
    geom_tile(aes(fill = auc)) + 
    geom_text(aes(label = ifelse(is.na(auc), "N.A.", as.character(round(auc, 3))))) +
...
```{r}
load("/storage/holab/chrissy/Ho_DCATS/simulation/one_increase.RData")
```
...
sim4_mainHM2 = res %>% 
  ...
  theme(axis.title.y = element_blank(),
        ...
        axis.title.x = element_blank(),
        legend.position = "bottom") +
  labs(fill='value') + ggtitle("Special case 2")
sim4_mainHM2
```
...
```{r}
(p1 + sim2_prop + curve + theme(legend.position="top"))/(sim2_mainHM1 + sim2_mainHM2 + sim2_mainHM3 + sim2_mainHM4  + plot_layout(ncol = 4))/(sim4_mainHM1 + sim4_mainHM2 + sim3_main)
