...
for (i in 1:cluster_num){
  ## cluster Info
  cluster <- SPARSim_create_simulation_parameter(
    intensity = Chu_param_preset[[i]]$intensity, 
    variability = Chu_param_preset[[i]]$variability, 
    library_size = sample(Chu_param_preset[[i]]$lib_size, size = round(batch_size*cell.prop[1])*batch_num, replace = TRUE),
  ...
  set.seed(123)
  sim_result <- SPARSim_simulation(dataset_parameter = cluster_params, batch_parameter = batch_parameter)
  sim_resultL = c(sim_resultL, sim_result)
}
