...
oper <- foreach(sim = 1:simulation_times, .packages=c("tidyverse", "Seurat", "DCATS", "speckle", "diffcyt", "scDC", "tidymodels", "miloR", "SingleCellExperiment"), .verbose = TRUE) %dopar% {
  ...
  timeL = rep(NA, 8) # DCATS w/wto similarity matrix, fisher, speckle, diffcyt, scDC
  ## simulation
  simulation = simulator_propM(totalV = rep(sample_size1, nrow(prop_mat)), prop_mat, setresolu, sim_mat, design_mat)
  if (is.na(simulation)[1] == TRUE){
    return(NA)
  }
  result$true_count = simulation$numb_sim
  ## data convert
  ...
  wtoPhi_emK_wtoDesign = dcats_GLM(sim_count, sim_design, simil_matK, base_model = "NULL")
  timeL[3] = Sys.time() - t3start
  ## Uniform
  ...
  t7start = Sys.time()
  res_scDC <- scDC_noClustering(cellTypes = simulation$dfRes$clusterRes, simulation$dfRes$batch, calCI = TRUE, calCI_method = c("percentile", "BCa", "multinom"),nboot = 1000, verbose = FALSE)
  ## scDC only allow input of a vector of condition
  ...
  if ("milo_more" %!in% colnames(miloDF_temp)) {miloDF_temp$milo_more = rep(0, nrow(miloDF_temp))}
  miloDF = miloDF_temp %>% 
    dplyr::select(cluster, milo_less, neutral, milo_more, detect)
  ## results
  miloDF_cond_full = miloDF %>% 
    filter(detect == "condition_full") %>% 
    dplyr::rename(full_milo_less = milo_less) %>% 
    dplyr::rename(full_neutral = neutral) %>% 
    dplyr::rename(full_milo_more = milo_more) %>% 
    dplyr::select(-detect)
  miloDF_cond_null = miloDF %>% 
    filter(detect == "condition_null") %>% 
    dplyr::rename(null_milo_less = milo_less) %>% 
    dplyr::rename(null_neutral = neutral) %>% 
    dplyr::rename(null_milo_more = milo_more) %>% 
    dplyr::select(-detect)
  cluster_map = data.frame(cluster = colnames(numb_cond1[,order2]), truth = truthRes) %>% 
        arrange(cluster)
      conditionDF = cluster_map %>% 
        ...
        merge(speckleRes, by = "cluster") %>% 
        merge(diffcytP, by = "cluster") %>% 
        left_join(miloDF_cond_full, by = "cluster") %>% 
        left_join(miloDF_cond_null, by = "cluster") %>% 
        replace(is.na(.), 0)
      result$conditionDF = conditionDF
      miloDF_gender_full = miloDF %>% 
        filter(detect == "gender_full") %>% 
        dplyr::rename(full_milo_less = milo_less) %>% 
        dplyr::rename(full_neutral = neutral) %>% 
        dplyr::rename(full_milo_more = milo_more) %>% 
        dplyr::select(-detect)
      miloDF_gender_null = miloDF %>% 
        filter(detect == "gender_null") %>% 
        dplyr::rename(null_milo_less = milo_less) %>% 
        dplyr::rename(null_neutral = neutral) %>% 
        dplyr::rename(null_milo_more = milo_more) %>% 
        dplyr::select(-detect)
      genderDF = data.frame(cluster = colnames(numb_cond1[,order2]), truth = rep(c("P", "P", "N", "N"), 2)) %>%
        ...
        left_join(miloDF_gender_full, by = "cluster") %>% 
        left_join(miloDF_gender_null, by = "cluster") %>% 
        replace(is.na(.), 0)
      result$genderDF = genderDF
      miloDF_age_full = miloDF %>% 
        filter(detect == "age_full") %>% 
        dplyr::rename(full_milo_less = milo_less) %>% 
        dplyr::rename(full_neutral = neutral) %>% 
        dplyr::rename(full_milo_more = milo_more) %>% 
        dplyr::select(-detect)
      miloDF_age_null = miloDF %>% 
        filter(detect == "age_null") %>% 
        dplyr::rename(null_milo_less = milo_less) %>% 
        dplyr::rename(null_neutral = neutral) %>% 
        dplyr::rename(null_milo_more = milo_more) %>% 
        dplyr::select(-detect)
      ageDF = data.frame(cluster = colnames(numb_cond1[,order2]), truth = rep(c("P", "P", "N", "N"), 2)) %>%
        ...
        left_join(miloDF_age_full, by = "cluster") %>% 
        left_join(miloDF_age_null, by = "cluster") %>% 
        replace(is.na(.), 0)
      result$ageDF = ageDF
      result$time = timeL
  return(result)
...
#for (idx in 11:30){
  print(str_c("idx: ", as.character(idx)))
  result <- list()  # where to save output
  timeL = rep(NA, 8) # DCATS w/wto similarity matrix, fisher, speckle, diffcyt, scDC
  ## simulation
  simulation = simulator_propM(totalV = rep(sample_size1, nrow(prop_mat)), prop_mat, setresolu, sim_mat, design_mat)
  if (is.na(simulation)[1] == TRUE){
...
file_name = str_c("/storage/holab/linxy/DCATS/simulation/replicates10&10_K", as.character(cluster_num), "_con", as.character(concentration), "_", cell_pool, more_negative, sample_size1, "&", sample_size2, "_para_test.RData")
save(oper, design_mat, add_mat, prop_mat, file = file_name)
```
...
for (file in fileL) {
  ...
  if("full_milo_less" %in% colnames(simulationDF)) {
    methods = c(methods, "milo_full", "milo_null")
    simulationDF = simulationDF %>% 
      mutate(full_milo_main = ifelse(full_milo_less > full_milo_more, full_milo_less, full_milo_more),
             full_milo_pct = full_milo_main/(full_milo_less + full_neutral + full_milo_more),
           full_milo_diff = abs(full_milo_less - full_milo_more)) %>% 
      dplyr::select(-full_milo_less, -full_milo_more, - full_neutral, -full_milo_main) %>% 
      mutate(null_milo_main = ifelse(null_milo_less > null_milo_more, null_milo_less, null_milo_more),
             null_milo_pct = null_milo_main/(null_milo_less + null_neutral + null_milo_more),
           null_milo_diff = abs(null_milo_less - null_milo_more)) %>% 
      dplyr::select(-null_milo_less, -null_milo_more, - null_neutral, -null_milo_main)
  }
  numb_mthd = length(methods)
  mcc = rep(NA, numb_mthd)
  auc = rep(NA, numb_mthd)
  ...
  for (i in 4:(3+numb_mthd)) {
    pred = simulationDF[,i]
    if (colnames(simulationDF)[i] == "full_milo_pct") {
      pred_res = ifelse(simulationDF$full_milo_diff > 0, "P", "N")
      auc[i-3] = getROC(simulationDF$truth, 1-pred)$auc
      prauc[i-3] = getPRC(simulationDF$truth, 1-pred)$prauc}
    else if (colnames(simulationDF)[i] == "null_milo_pct") {
      pred_res = ifelse(simulationDF$null_milo_diff > 0, "P", "N")
      auc[i-3] = getROC(simulationDF$truth, 1-pred)$auc
      prauc[i-3] = getPRC(simulationDF$truth, 1-pred)$prauc
    } else {
      pred_res = ifelse(pred < 0.1, "P", "N")
...
```{r}
file_name = str_c("/storage/holab/linxy/DCATS/simulation/replicates10&10_K", as.character(cluster_num), "_con", as.character(concentration), "_", cell_pool, more_negative, sample_size1, "&", sample_size2, "_para_testResDF.RData")
save(res_all, file = file_name)
```
