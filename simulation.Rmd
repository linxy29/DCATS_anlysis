...
save(simulationDF_list, phiDF, numb_cond1M, numb_cond2M, numb_cond1BCM, numb_cond2BCM, file = "/storage/holab/linxy/DCATS/simulation/toy_simulation0117.RData")
```
...
oper <- foreach(sim = 1:simulation_times, .packages=c("tidyverse", "Seurat", "DCATS", "speckle", "diffcyt", "scDC", "tidymodels", "miloR", "SingleCellExperiment"), .verbose = TRUE) %dopar% {
  ...
  cluster_map = data.frame(cluster = colnames(numb_cond1[,order2]), truth = truthRes) %>% 
      ...
      ageDF = data.frame(cluster = colnames(numb_cond1[,order2]), truth = c("P", "N", "N", "P", "P", "N", "N", "P")) %>%
        arrange(cluster) %>% 
