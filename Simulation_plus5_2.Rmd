...
simulationDF = data.frame()
for (i in 1:50){
  simil_mat = get_similarity_mat(K, confuse_rate=0.3)
  sim_dat <- DCATS::simulator_base(totals1, totals2, diri_s1, diri_s2, simil_mat)
...
graphics::text(0.2, 0.68, paste(methods[1]), col=1)
for (i in 2:8){
  ...
  graphics::text(0.2, 0.68-(i-1)*0.07, paste(methods[i]), col=i)
}
```

### 3 clusters(add misclassification twice)
...
totals1 = ceiling(runif(4, 500, 2000))
totals2 = ceiling(runif(2, 500, 2000))
diri_s1 = c(1/3, 1/3, 1/3) * 50
diri_s2 = c(1/3, 1/2, 1/6) * 50
simil_mat = get_similarity_mat(K, confuse_rate=0.2)
sim_dat <- DCATS::simulator_base(totals1, totals2, diri_s1, diri_s2, simil_mat)
```
