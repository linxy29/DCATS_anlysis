...
res = data.frame(method = c("ref2", "ref3", "org", "truth2", "truth3"), mcc = mcc, auc = auc, prauc = prauc, sensitivity = sensitivity, specificity = specificity, precision = precision, F1 = F1) %>% 
  arrange(desc(auc))
```
...
for (idx in 1:simulation_times) {
  ...
  #emSVM_truth3 = dcats_new(sim_count, sim_design, simil_matSVM, fix_phi = phi, reference = colnames(numb_cond1[,order2])[c(6,7,8)])
  emSVM_truth2 = dcats_new(sim_count, sim_design, simil_matSVM, fix_phi = phi, reference = colnames(numb_cond1[,order2])[c(7,8)])
  #emK_truth3 = dcats_new(sim_count, sim_design, simil_matK, fix_phi = phi, reference = colnames(numb_cond1[,order2])[c(6,7,8)])
  emK_truth2 = dcats_new(sim_count, sim_design, simil_matK, fix_phi = phi, reference = colnames(numb_cond1[,order2])[c(7,8)])
...
truthRes = c("N", "N", "N", "N", "N", "N", "P", "P")
#print(probC2)
file_name = "/storage/holab/linxy/DCATS/simulation/current/two_increase_toy.RData"
```
