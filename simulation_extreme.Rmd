...
for (idx in 1:simulation_times) {
  ...
  reference_orderL[[idx]] = data.frame(ref_order = ref_order, knnref_order = knnref_order, svmref_order = svmref_order)
  ...
  emSVM_truth3 = dcats_new(sim_count, sim_design, simil_matSVM, fix_phi = phi, reference = c(6,7,8))
  emSVM_truth2 = dcats_new(sim_count, sim_design, simil_matSVM, fix_phi = phi, reference = c(7,8))
  emK_truth3 = dcats_new(sim_count, sim_design, simil_matK, fix_phi = phi, reference = c(6,7,8))
  emK_truth2 = dcats_new(sim_count, sim_design, simil_matK, fix_phi = phi, reference = c(7,8))
...
```{r}
knn_temp = data.frame(order = orderDF$knnref_order, idx = orderDF$knn_idx) %>% 
  mutate(method = "knn")
svm_temp = data.frame(order = orderDF$svmref_order, idx = orderDF$svm_idx) %>% 
  mutate(method = "svm")
null_temp = data.frame(order = orderDF$ref_order, idx = orderDF$org_idx) %>% 
  mutate(method = "null")
rbind(knn_temp, svm_temp, null_temp) %>% 
  ggplot(aes(x = order, y = idx, color = method)) +
  geom_boxplot()
```


```{r}
for (celltype in c("A", "B", "C", "D", "E", "F", "G", "H")){
  print(celltype)
  print(str_c("Cell type: ", celltype))
  print("data from: null, knn, svm")
  orderDF %>% filter(ref_order == celltype) %>% .$org_idx %>% summary() %>% print()
  orderDF %>% filter(knnref_order == celltype) %>% .$knn_idx %>% summary() %>% print()
  orderDF %>% filter(svmref_order == celltype) %>% .$svm_idx %>% summary() %>% print()
}
...
seurat_countDF %>% 
  ...
  pivot_longer(A:C, names_to = "cellType", values_to = "count") %>% 
