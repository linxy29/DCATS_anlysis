...
```{r}
res_mat = rbind(numb_ms1Nat, numb_ms1)
count_mat = res_mat %>% select(B01:T11)
design_mat = res_mat %>% select(sex, age = age_sampling, treatment = natalizumab_treatment)
...
```{r}
Peng2018 <- read.delim("C:/#Code/R/Ho_DCATS/data/real_world/Peng2018_meta.txt") %>% 
  janitor::clean_names() %>% 
