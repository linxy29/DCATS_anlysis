...
## function: new version of DCATS (allows more than one reference group)
dcats_new = function (count_mat, design_mat, similarity_mat = NULL, pseudo_count = NULL, 
...
getANCOMBC = function(assay_data, col_data){
  #library(ANCOMBC)
...
dcats_bc = function(count_mat, similarity_mat){
  ## using estimated the latent cell counts
  count_use = count_mat
  if(!is.null(similarity_mat)) {
    for (i in seq_len(nrow(count_mat))) {
      count_use[i, ] <- sum(count_mat[i, ]) *
        DCATS::multinom_EM(count_mat[i, ], similarity_mat, verbose = FALSE)$mu
    }
  }
  return(round(count_use))
}