...
```{r}
load("/storage/holab/linxy/DCATS/simulation/toy_simulation0209.RData")
...
cond1DF = rbind(prop_cond1M, prop_cond1BCM, prop_cond2M, prop_cond2BCM, prop_true1M, prop_true2M) %>% as.data.frame() %>% 
  mutate(state = c(rep("with Misclassification", nrow(numb_cond1M)-1), rep("with Bias Correction", nrow(numb_cond1BCM)-1), rep("with Misclassification", nrow(numb_cond2M)-1), rep("with Bias Correction", nrow(numb_cond2BCM)-1), rep("no Misclassification", nrow(prop_true1M) + nrow(prop_true2M))),
  ...
  mutate(state = factor(state, levels = c("no Misclassification", "with Misclassification", "with Bias Correction")))
colnames(cond1DF) = c("A", "B", "C", "state", "cond")
```

```{r}
sim_prop = cond1DF %>% 
  filter(state != "with Bias Correction") %>% 
...
```{r}
load("/storage/holab/linxy/DCATS/simulation/clusterN/replicates3&3_K10_con100_splatter3000&3000para.RData")
```
...
for (idx in 1:length(oper)){
    if (!is.na(oper[[idx]])) {
      ...
      oper[[idx]]$true_count = oper[[idx]]$true_count %>% matrix(ncol = 10) %>% as.data.frame() %>% mutate(condition = c(rep("Condition 1", 3), rep("Condition 2", 3)))
      true_count = rbind(true_count, oper[[idx]]$true_count)
      }
}
colnames(seurat_count) = c("N1", "N2", "N3", "N4", "N5", "P1", "P2", "P3", "P4", "P5", "condition")
colnames(true_count) = c("N1", "N2", "N3", "N4", "N5", "P1", "P2", "P3", "P4", "P5", "condition")
```

```{r}
sim2_prop = true_count %>% 
  pivot_longer(N1:P5, names_to = "cellType", values_to = "count") %>% 
...
sim2_auc = ggroc(list(DCATS=rocobj_dcats, Fisher=rocobj_fisher, diffcyt=rocobj_diffcyt, speckle=rocobj_speckle, scDC=rocobj_scDC, milo=rocobj_milo))+
  geom_point(aes(x=0.858, y=0.658), colour="black", shape = 4) +
  geom_point(aes(x=0.892, y=0.517), colour="black", shape = 4) +
  geom_point(aes(x=0.917, y=0.517), colour="black", shape = 4) +
  geom_point(aes(x=0.775, y=0.525), colour="black", shape = 4) +
  geom_point(aes(x=0.258, y=0.892), colour="black", shape = 4) +
  geom_point(aes(x=0.258, y=0.792), colour="black", shape = 4) +
  labs(color='method')
sim2_auc
```
...
save(nhoodsRes_all, file = "D:/Data/DCATS/simulation/nhoodsRes_all_clustersN.RData")
```
...
save(stat.test, file = "./data/real_world/Ren2021.RData")
```
