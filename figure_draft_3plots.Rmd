...
```{r}
sim1_main + sim1_main
ggsave("./plot/Fig1.png", bg = "transparent", width = 12, height = 4)
```
...
res_all %>% 
  ...
  dplyr::select(method, auc, prauc, mcc, F1, sensitivity, specificity, precision, replicates) %>% 
  pivot_longer(auc:precision, names_to = "statistics", values_to = "value") %>% 
  group_by(replicates, statistics) %>% 
...
res_all %>% 
  ...
  #mutate(method = ifelse(method == "fisher", "Fisher", method)) %>% 
  mutate(method = factor(method, level = c("scDC", "milo", "wtoPhi_emK", "betabin_null", "speckle", "diffcyt", "fisher", "estPhi_emK", "estPhi_null"))) %>% 
  dplyr::select(method, auc, prauc, mcc, F1, sensitivity, specificity, precision, clustersN) %>% 
  pivot_longer(auc:precision, names_to = "statistics", values_to = "value") %>% 
  group_by(clustersN, statistics) %>% 
  mutate(value_order = 10-order(order(value))) %>% 
  ggplot(aes(clustersN, method)) +
...
```{r}
(p1 + p2 + p3)/((sim2_mainHM1 + sim2_mainHM2) + theme(legend.position="bottom") + sim3_main)
