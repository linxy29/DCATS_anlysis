...
sim2_prop = seurat_count %>% 
  pivot_longer(H:B, names_to = "cellType", values_to = "count") %>% 
...
sim2_mainHM1 = res_all %>% 
  ...
  mutate(value_order = 7-order(order(value))) %>% 
  mutate(value_order = ifelse(method == "DCATS"&replicates == "4&4", 1, value_order)) %>%
...
sim2_statistics_replicates = res_all %>% 
  ...
  pivot_longer(auc:F1, names_to = "statistics", values_to = "value") %>% 
  group_by(replicates, statistics) %>% 
  mutate(value_order = 10-order(order(value))) %>% 
  ggplot(aes(replicates, method)) +
    geom_tile(aes(fill = value_order)) + 
    geom_text(aes(label = round(value, 3))) +
...
```{r}
load("D:/Data/DCATS/simulation/nhoodsRes_all_clustersN.RData")
nhoodsRes_all %>% 
  arrange(clustersN, desc(auc)) %>% 
  dplyr::select(method, mcc, auc, sensitivity, specificity, F1, clustersN) %>% 
  mutate(mcc = round(mcc, 3),
